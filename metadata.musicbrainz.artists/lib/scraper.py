...
import xbmcplugin
import xbmcaddon
#from lib.theaudiodb import theaudiodb_artistdetails
#from lib.theaudiodb import theaudiodb_artistalbums
from lib.musicbrainz import musicbrainz_artistfind
#from lib.musicbrainz import musicbrainz_artistdetails
from lib.musicbrainz2 import musicbrainz_arstistdetails2
from lib.discogs import discogs_artistfind
from lib.discogs import discogs_artistdetails
from lib.discogs import discogs_artistalbums
from lib.allmusic import allmusic_artistdetails
from lib.nfo import nfo_geturl
#from lib.fanarttv import fanarttv_artistart
import lib.utils 
...
class Scraper(object):
	...
	def find_artist(self, artist, site):
		...
		# musicbrainz
		if site == 'musicbrainz':
			url = lib.utils.MUSICBRAINZURL % (lib.utils.MUSICBRAINZSEARCH % urllib.quote_plus(artist))
			scraper = musicbrainz_artistfind
		# musicbrainz
		if site == 'discogs':
			url = lib.utils.DISCOGSURL % (lib.utils.DISCOGSSEARCH % (urllib.quote_plus(artist), lib.utils.DISCOGSKEY , lib.utils.DISCOGSSECRET))
	...
	def get_details(self, param, site, details):
		if site == 'musicbrainz':
			albumresults = musicbrainz_arstistdetails2(param)
			if not albumresults:
				return
		...
		# discogs
			...
			url = lib.utils.DISCOGSURL % (lib.utils.DISCOGSDETAILS % (param, lib.utils.DISCOGSKEY, lib.utils.DISCOGSSECRET))
			artistscraper = discogs_artistdetails
			result = get_data(url, True)
			artistresults = artistscraper(result)
			albumsurl = lib.utils.DISCOGSURL % (lib.utils.DISCOGSDISCOGRAPHY % (param, lib.utils.DISCOGSKEY, lib.utils.DISCOGSSECRET))
			scraper = discogs_artistalbums
			albumdata = get_data(albumsurl, True)
			if albumdata:
